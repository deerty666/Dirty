document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ تصحيح المعرّف هنا ليتوافق مع 'cart-modal' في HTML
    ...
    function addToCart(itemId, name, basePrice, options = {}) {
        // حساب السعر النهائي للوجبة الواحدة بما في ذلك خياراتها
        let finalPricePerUnit = basePrice;
        for (const key in options) {
            finalPricePerUnit += options[key].price;
        }
        ...
        if (existingItem) {
            ...
            cart.push({ id: itemId, name, price: finalPricePerUnit, quantity: 1, options });
        }
    ...
    // وظيفة الإضافة إلى السلة (مُعدلة لاستخدام خصائص data-* الجديدة)
    document.querySelectorAll('.add-to-cart-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            // 💡 استخراج السعر الأساسي من خاصية data-base-price على البطاقة
            ...
            if (radioButtons.length > 0) {
                radioButtons.forEach(radio => {
                    ...
                    // 💡 استخراج اسم الخيار من قيمة الإدخال (value)
                    const optionName = radio.value; 
                    // 💡 استخراج سعر الخيار الإضافي من خاصية data-option-price
