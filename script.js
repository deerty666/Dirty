document.addEventListener('DOMContentLoaded', () => {
    ...
    // ** الإضافات الجديدة للتحكم في شاشة الترحيب **
    const SPLASH_SCREEN = document.getElementById('splash-screen');
    const MAIN_CONTENT = document.getElementById('main-app-content'); 

    // =========== 0. وظيفة التحكم في شاشة الترحيب (الجديدة) ===========
    
    // إظهار الشاشة الترحيبية لمدة 1.5 ثانية
    setTimeout(() => {
        // الخطوة 1: بدء تلاشي الشاشة الترحيبية (Opacity 0)
        SPLASH_SCREEN.style.opacity = '0'; 
        
        // الخطوة 2: بعد انتهاء التلاشي (0.5 ثانية حسب CSS)، إخفاءها بالكامل وإظهار المحتوى
        setTimeout(() => {
            SPLASH_SCREEN.style.display = 'none';
            if (MAIN_CONTENT) { 
                // إظهار محتوى التطبيق الذي كان مخفياً بـ (display: none)
                MAIN_CONTENT.style.display = 'block'; 
            }
        }, 500); 

    }, 1500); // إبقاء شاشة البداية ظاهرة لمدة 1.5 ثانية (1500 مللي ثانية)

    // =========== 1. وظائف السلة (كما كانت سابقاً) ===========
